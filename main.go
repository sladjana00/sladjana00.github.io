...
var ADTCPOpenPortsClients []string
var ADTCPOpenPortsServers []string
...
var nbrTotalServer int = 0
var nbrClientWithOpenPort int = 0
var nbrTotalClient int = 0
...
func getTcpOpenPorts(ADCsvResults [][]string) []string {
	...
	fmt.Println("List of all opened ports =>>", TCPports)
...
func GetADOSPerc(w http.ResponseWriter, r *http.Request) {
	...
	if totalHostsAD != 0 {
		var Data []DataOS
		for _, os := range treeOS.Content {
			dataos := DataOS{
				OS:   os.OsName,
				Perc: float32(100*len(os.SonsLvl1)) / float32(totalHostsAD),
			}
			Data = append(Data, dataos)
		}

		json.NewEncoder(w).Encode(Data)
	} else {
		json.NewEncoder(w).Encode(DataOS{OS: "", Perc: 0})
	}
}

func GetADPortsPercServers(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("List of Open ports for servers only : ", ADTCPOpenPortsServers)

	var Data []DataPort
	for _, port := range treePorts.Content {
		var _perc float32 = 0

		if nbrTotalServer != 0 {
			_perc = float32(100*len(port.Servers)) / float32(len(ADTCPOpenPortsServers))
		}

		portdescription := strings.Split(port.Port, ":")
		dataport := DataPort{
			Port: portdescription[0],
			Perc: _perc,
		}
		Data = append(Data, dataport)
	}

	var TmpData = Data
	var FinalData []DataPort
	// Order the Data slice
	for i := 0; i < len(Data); i++ {

		// Find the max perc value in the slice
		var maxPerc float32 = 0
		var maxPort string
		var maxIndx int = 0
		for indexTmp, tmp := range TmpData {
			if tmp.Perc > maxPerc {
				maxPerc = tmp.Perc
				maxPort = tmp.Port
				maxIndx = indexTmp
		...
		TmpData = append(TmpData[:maxIndx], TmpData[maxIndx+1:]...)

		FinalData = append(FinalData, DataPort{Perc: maxPerc, Port: maxPort})
	}

	json.NewEncoder(w).Encode(FinalData)
}

func GetADPortsPercClients(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("List of Open ports for clients only : ", ADTCPOpenPortsClients)
	var Data []DataPort
	for _, port := range treePorts.Content {
		...
		if nbrTotalClient != 0 {
			_perc = float32(100*len(port.Clients)) / float32(len(ADTCPOpenPortsClients))
		}
...
func GetADPortsPercUnknown(w http.ResponseWriter, r *http.Request) {
	...
	for _, port := range treePorts.Content {
		...
		if totalHostsAD == 0 {
			_perc = float32(100*len(port.Clients)) / float32(len(LocalTCPOpenPorts))
		} else {
			_perc = 0
		}
...
func PreparePortsSummary() {
	...
	if machineIsDomainConnected() {
		...
		allOpenPorts := unique(getTcpOpenPorts(ADCsvResults))
		for _, port := range allOpenPorts {
			for _, row := range IPData {
				if strings.Contains(row.TCPPorts, port) {
					if row.Type == "Client" {
						ADTCPOpenPortsClients = append(ADTCPOpenPortsClients, port)
					} else if row.Type == "Server" {
						ADTCPOpenPortsServers = append(ADTCPOpenPortsServers, port)
					}
				}
			}
		}

		openPorts := unique(getTcpOpenPorts(ADCsvResults))
		for _, port := range openPorts {
			...
			PortElmnt := PortsLvl1{
				Port:    port + "/tcp:" + getPortServiceByPortNumber(port),
				Clients: _Clients,
				Servers: _Servers,
			}
		...
		openPorts := unique(getTcpOpenPorts(LocalCsvResults))
...
func SaveReport(w http.ResponseWriter, r *http.Request) {
	...
	//shellCmd1 := "&'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe' @('C:/Users/Amine/Desktop/goscanner/index.html')"
...
*/
func SendReportInMail(w http.ResponseWriter, r *http.Request) {
	enableCors(&w)

	//out, err := executePowerShell("Start-Process 'mailto:dhub01@domain.tld?Subject=This is a Subject&Attachment='C:/Users/Amine/Downloads/Tasks.pdf'&Body=This is the contents of the message'")
...
func handleRequests() {
	...
	http.HandleFunc("/getADOSPerc", GetADOSPerc)
	http.HandleFunc("/getADPortsSummary", GetADPortsSummary)
	http.HandleFunc("/getADPortsPercServers", GetADPortsPercServers)
	http.HandleFunc("/getADPortsPercClients", GetADPortsPercClients)
	http.HandleFunc("/getADPortsPercUnknown", GetADPortsPercUnknown)
	http.HandleFunc("/openNetworkDetails", OpenNetworkDetails)
	http.HandleFunc("/openAssetsDetails", OpenAssetsDetails)
	http.HandleFunc("/saveReport", SaveReport)
	http.HandleFunc("/sendReportInMail", SendReportInMail)
...
func main() {
	...
	fmt.Println(treeOS)
	fmt.Println(treeIP)
	fmt.Println(treePorts)
	fmt.Println("**************************************************************************************")
	fmt.Println("**************************************************************************************")
	fmt.Println("**************************************************************************************")

	// Now Processing the scan results to html and xlsx files.
	if machineIsDomainConnected() {
		...
		nbrAssets = strconv.Itoa(nbrAccessibleHostsAD)
